# Error Report 20260205T223526Z

## Context
aiogram update error: {"update_id":896307104,"message":{"message_id":47,"date":1770330915,"chat":{"id":935139103,"type":"private","username":"rayeeev","first_name":"Erden"},"from_user":{"id":935139103,"is_bot":false,"first_name":"Erden","username":"rayeeev","language_code":"en"},"text":"Let’s switch to English"}}
...
| Symptom | Why it happened | Where it originates |
|---------|----------------|---------------------|
| `google.genai.errors.ClientError: 429 RESOURCE_EXHAUSTED` | The Gemini API free‑tier quota (`generativelanguage.googleapis.com/generate_content_free_tier_requests`) is limited to **20 requests per day per project per model**. Your bot has already sent more than 20 `generate_content` calls to the `gemini‑3‑flash` model, so the service returns HTTP 429 with a `Retry‑After` hint (`36 s`). | `src/llm/gemini_client.py → _generate_with_retry → google.genai.models.generate_content` (wrapped by `tenacity` retry decorator). |

The exception bubbles up through the async call chain and finally crashes the handler because the `ErrorsMiddleware` only forwards the exception to an *error* router – you have no error handler that deals with a quota‑exhaustion error.

---

## Minimal patch plan

### 1. **Add a graceful‑degradation path for 429**

Catch the `ClientError` that carries a 429 status, read the `Retry‑Info` header (or the `retryDelay` field in the JSON), wait that amount, and either:

* **Retry** a limited number of times **or**
* **Return a fallback response** (e.g. “I’m currently out of capacity, please try again later.”)
...
If you prefer **no retry** and just a user‑friendly message:
